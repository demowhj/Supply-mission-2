var helicopterIMG, helicopterSprite, packageSprite,packageIMG;
var packageBody,ground;
var box1, box2, box3, bs1, bs2, bs3;

const Engine = Matter.Engine;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Body = Matter.Body;

function preload()
{
	helicopterIMG=loadImage("helicopter.png")
	packageIMG=loadImage("package.png")
}

function setup() {
	createCanvas(800, 700);
	rectMode(CENTER);

	packageSprite=createSprite(width/2, 80, 10,10);
	packageSprite.addImage(packageIMG)
	packageSprite.scale=0.15;

	bs1 = createSprite(width/2, height-60, 200, 20);
	bs2 = createSprite(width/2 - 100, height-100, 20, 100);
	bs3 = createSprite(width/2 + 100, height-100, 20, 100);

	bs1.shapeColor = "red";
	bs2.shapeColor = "red";
	bs3.shapeColor = "red";

	helicopterSprite=createSprite(width/2, 200, 10,10);
	helicopterSprite.addImage(helicopterIMG)
	helicopterSprite.scale=0.6

	groundSprite=createSprite(width/2, height-35, width,10);
	groundSprite.shapeColor=color(255)


	engine = Engine.create();
	world = engine.world;

	packageBody = Bodies.circle(width/2 , 200 , 5 , {restitution:0.5, isStatic:true});
	World.add(world, packageBody);
	

	//Create a Ground
	ground = Bodies.rectangle(width/2, 650, width, 10 , {isStatic:true} );
 	World.add(world, ground);


	Engine.run(engine);
  
	box1 = Bodies.rectangle(width/2, height-60, 200, 20, {isStatic:true});
	box2 = Bodies.rectangle(width/2 - 100, height-100, 20, 100, {isStatic:true});
	box3 = Bodies.rectangle(width/2 + 100, height-100, 20, 100, {isStatic:true});

	World.add(world, box1);
	World.add(world, box2);
	World.add(world, box3);
}


function draw() {
  	rectMode(CENTER);
  	background(0);
  	packageSprite.x= packageBody.position.x; 
	packageSprite.y= packageBody.position.y; 
	  
	bs1.x= box1.position.x; 
	bs1.y= box1.position.y; 

	bs2.x= box2.position.x; 
	bs2.y= box2.position.y; 

	bs3.x= box3.position.x; 
	bs3.y= box3.position.y; 
  	drawSprites();

  	// fill("red");
	// rect(box1.position.x, box1.position.y, 200, 20);
	// rect(box2.position.x, box2.position.y, 20, 100);
	// rect(box3.position.x, box3.position.y, 20, 100);
 
}

function keyPressed() {
 if (keyCode === DOWN_ARROW) {
    Matter.Body.setStatic(packageBody, false);
  }
}



